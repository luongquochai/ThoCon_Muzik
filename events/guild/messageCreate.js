...
module.exports = async (client, message) => {
  ...
  if (messageChannel == musicChannel) {

    let prefix = client.settings.get(message.guild.id, `prefix`)
    const prefixRegex = new RegExp(`^(<@!?${client.user.id}>|${escapeRegex(prefix)})`);
    let args = [];
    if (!prefixRegex.test(message.content)) {
      let a = ['-p']; //set default message is play music with prefix !p
      a.push(message.content); // ['!p', 'xxx']
      args = a.join(' ').slice(1).trim().split(/ +/).filter(Boolean); // ['p', 'xxx']
    } else {
      const [, mPrefix] = message.content.match(prefixRegex);
      args = message.content.slice(mPrefix.length).trim().split(/ +/).filter(Boolean);
    }
    ...
    if (command) {
      ...
      //Check if user is on cooldown with the cmd, with tibbaR#8979's Function from /handlers/functions.js
