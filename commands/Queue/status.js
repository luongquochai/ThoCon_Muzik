		...
		try {
			...
			try {
				...
				let embed = new MessageEmbed().setColor(ee.color)
					// .setDescription(`See the [Queue on the **DASHBOARD** Live!](http://dashboard.Muzik.eu/queue/${newQueue.id})`)
					.addField(`💡 Yêu cầu bởi:`, `>>> ${newTrack.user}`, true)
					.addField(`⏱ Thời lượng:`, `>>> \`${newQueue.formattedCurrentTime} / ${newTrack.formattedDuration}\``, true)
					.addField(`🌀 Danh sách:`, `>>> \`${newQueue.songs.length} song(s)\`\n\`${newQueue.formattedDuration}\``, true)
					.addField(`🔊 Âm lượng:`, `>>> \`${newQueue.volume} %\``, true)
					.addField(`♾ Lặp lại:`, `>>> ${newQueue.repeatMode ? newQueue.repeatMode === 2 ? `${client.allEmojis.check_mark} \`Queue\`` : `${client.allEmojis.check_mark} \`Song\`` : `${client.allEmojis.x}`}`, true)
					...
					.setFooter(`💯 ${newTrack.user.tag} | powered by lqhaiii`, newTrack.user.displayAvatarURL({
						dynamic: true
					}));
				message.reply({
					embeds: [embed]
